...
# Assign parents
stack = [] # holds (level, id)
for sec in sections:
    lvl = sec["level"]
    # If this is an intro, we'll keep level 0
    # Pop anything with a higher or equal level to find the parent
    while stack and stack[-1][0] >= lvl:
        stack.pop()
        
    if stack:
        sec["parent_id"] = stack[-1][1]
    else:
        sec["parent_id"] = None
...
for sec in sections:
    ...
    if sec["parent_id"]:
        edges_json.append({
            "id": uuid.uuid4().hex[:10],
            "fromNode": sec["parent_id"],
            "fromSide": "bottom",
            "toNode": sec["id"],
            "toSide": "top" # Connect bottom of parent to top of child (or left side of child)
            # wait, if parent is way above, a top connection looks good. 
        })
